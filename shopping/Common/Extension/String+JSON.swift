...
extension String {
    ...
    /// 通过key，提取get请求的某个参数
    func getUrlValue(_ parameterKey: String)->String {
        let pattern = "(^|&|\\?)+\(parameterKey)=+([^&]*)(&|$)"
        let arrs = CommonUtil.regexGetSub(pattern: pattern, str: self)
        var string = arrs.last ?? ""
    ...
    /// 增加中划线
    func addLine()->NSAttributedString {
        let attribtStr = NSAttributedString(string: "\(self)", attributes: [NSAttributedString.Key.foregroundColor: Colors.randomColor, NSAttributedString.Key.strikethroughStyle: NSNumber(value: Int8(NSUnderlineStyle.single.rawValue))])
        return attribtStr
    }
