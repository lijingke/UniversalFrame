...
extension String {
    ...
    func stringInsert(content: String, locat: Int) -> String {
        ...
        let str1 = self.stringCut(end: locat)
        let str2 = self.stringCutToEnd(start: locat + 1)
...
public extension String {
    ...
    internal func getNormalStrH(strFont: CGFloat, w: CGFloat) -> CGFloat {
        return self.getNormalStrSize(str: self, font: strFont, w: w, h: CGFloat.greatestFiniteMagnitude).height
    }
    
    /** 获取字符串宽度W */
    internal func getNormalStrW(strFont: CGFloat, h: CGFloat) -> CGFloat {
        return self.getNormalStrSize(str: self, font: strFont, w: CGFloat.greatestFiniteMagnitude, h: h).width
    }
}
